...
class Banner(val bannerId  : Int ,
    ...
    private fun generateParams(): String {
        val shopIdParams = generateParamString("_shopId", Get4ClickSDK.getShopId().toString())
    ...
    private fun generateStagingHTML(scale : Double): String {
        val string = "<html>" +
                ...
                "this.lS=function(s){document.write('<sc'+'ript type=\"text/javascript\" src=\"'+s+'\" async=\"true\"></scr'+'ipt>');},\n" +
                "this.gc=function(){return document.getElementById(this.htmlElementId);};\n" +
                "var r=[];for(e in this.params){if(typeof(e)==='string'){r.push(e+'='+encodeURIComponent(this.params[e]));}}r.push('method=main');r.push('jsc=iPromoCpnObj');this.lS(('https:'==document.location.protocol ? 'https://':'http://')+'staging.get4click.ru/wrapper.php?'+r.join('&'));};\n" +
                "\n" +
                "var iPromoCpnObj = new _iPromoBannerObj();\n" +
                "</script>" +
                "</body>" +
